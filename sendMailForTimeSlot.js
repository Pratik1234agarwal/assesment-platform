...
formatSlot(startTime,endTime){
    let t1 = startTime.getHours();
    t1 = t1 > 12 ?`${t1 - 12}:${startTime.getMinutes()} PM `: `${t1}:${startTime.getMinutes()} AM`;
    let t2 = endTime.getHours();
    t2 = t2 > 12 ?`${t2 - 12}:${endTime.getMinutes()} PM `: `${t2}:${endTime.getMinutes()} AM`;
    return `${t1} - ${t2}`;
}
